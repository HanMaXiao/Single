...
class OperationLog(Model):
    ...
    level = fields.CharEnumField(OperationLevel, max_length=16, default=OperationLevel.INFO, description="日志级别")
...
class AlertRecord(Model):
    ...
    status = fields.CharEnumField(AlertStatus, max_length=16, default=AlertStatus.OPEN, description="告警状态")
    created_at = fields.DatetimeField(auto_now_add=True, description="创建时间")
