...
class PaymentRecord(Model):
    ...
    status = fields.CharEnumField(PaymentStatus, max_length=16, default=PaymentStatus.PENDING, description="支付状态")
    external_transaction_id = fields.CharField(max_length=128, null=True, index=True, description="外部交易号")
...
class RefundRecord(Model):
    ...
    status = fields.CharEnumField(RefundStatus, max_length=16, default=RefundStatus.PENDING, description="退款状态")
    reason = fields.TextField(null=True, description="退款原因")
    external_refund_id = fields.CharField(max_length=128, null=True, index=True, description="外部退款号")
